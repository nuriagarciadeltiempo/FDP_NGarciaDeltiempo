...
def vep_process_vcf(vcf_path, acmg_db_path):
    ...
    # Here in genotyping data we are computing how many samples are homozygous (full expression), heterozygous (50% chance of expression) and normals (non expressed)
...
def load_vep_databases(acmg_db_path, assembly, ref_anno, trio_data, pp2_percent, pp2_min, bp1_percent):
    ...
    with open(os.path.join(acmg_db_path, "final_PVS1.txt"), 'r') as pvs1_file:
        for line in pvs1_file:
            ...
            PVS1_list.append(new_line) # final_PVS1.txt contains a list of all the genes that has to be consider
    ...
    #'BS2_rec_dom_ad.txt'is a db previously imported.
    rec_list = []
    dom_list = []
    adult_list = []
            ...
            if conseq.split('&')[0] == 'stop_gained' or conseq.split('&')[0] == 'frameshift_variant':
                ...
                try:
                    if int(stop) - int(start) <= 110:  # To offset untranslated regions
                        PVS1_contrib.append(0) # not interesting if the change is small
                    else:
                ...
                #PVS1_contrib.append(1)
                  ...
                  or conseq.split('&')[0] == 'splice_donor_variant') and splice >= 0.6: # splice contains the max numb from ada and rf scores.
                PVS1_contrib.append(1)
            else:
    ...
    for chr, ref, alt, start, stop, aa_change, exo_func in zip(chr_series, ref_series, alt_series, start_series, end_series, aa_change_series, exonic_func_series):
        if exo_func == "missense_variant":
            if len(aa_change) == 3:# To avoid catching indels with aa changes
                ...
                last = aa_change.split('/')[1] # storing in two index variants the letter of the ref aminoacid and the alt one. In case they both are known to be pathogenic PS1 = 1
                try:
        ...
        for key in whole_dict: # for each family in the dictionary
            ...
            for mom, dad, child in zip(mother_series, father_series, offspring_series):
                if mom == '0/0' and dad == '0/0' and child == ('0/1' or '1/1' or '0/2' or '2/2'):
                    PS2_contrib_per_fam.append(1) # PS2 = 1 just if it is a de novo gene.
                else:
...
def check_vep_PS3(full_stuff):
    ...
    PS3_contrib = []
    for rev, sig in zip(review_series, sig_series): ## Changed numerical clasification to categorical
        if (rev.split('&')[0] == 'reviewed_by_expert_panel' or rev.split('&')[0] == 'practice_guideline') and \
                (sig.split('&')[0] == 'Pathogenic' or sig.split('&')[0] == 'drug_response'):
...
def check_vep_PP2(full_stuff, PP2_list, ref_anno):
    gene_series = full_stuff['SYMBOL'] # column SpliceAI_pred_SYMBOL
    type_series = full_stuff['Consequence'] # column ExonicFunc.refGene
...
def check_vep_PP5(full_stuff):
    ...
    PP5_contrib = []
    for pred in path_series: ## Changed numerical classification into categorical.
        if (pred.split('&')[0] == 'Pathogenic' or pred.split('&')[0] == 'Likely_pathogeic' or pred.split('&')[0] == 'Pathogenic/Likely_pathogenic') and \
                (pred.split('&')[0] != 'Conflicting_interpretations_of_pathogenicity'):  # TODO INCLUDE OR NOT CONFLICTING
...
def check_vep_BS1(full_stuff, cutoff): # cutoff is determined to be 0.005
    ...
    max_freq_series = full_stuff[filter_columns].replace('.', 0) # replacing every '.' found in colums for a numeric value: 0.
    ...
    max_freq_series = max_freq_series.max(axis=1) # selects the higher number of every row.
...
def check_vep_BS2(full_stuff, BS2_hom_het_dict, rec_list, dom_list, adult_list, ref_anno):
    ...
    for chr, start, ref, alt, gene, sex in all_tuple:

        if gene in adult_list:  # Remove all in adult onset
            BS2_contrib.append(0)

        elif gene in rec_list:  # Check for recessive first. #in case the name of the gene is has rec = 1 in 'BS2_rec_dom_ad.txt'
            key_snv = ref + '_' + alt 
            try:
                if key_snv == BS2_hom_het_dict["hom"][str(chr)][str(start)]: # dictionary related to 'BS2_hom_het' database
                    BS2_contrib.append(1) #if it is located in the dictionary and also recessive and homo
                else:
                    BS2_contrib.append(0) # in case it says it is recessive but in contrast it is heterozygous. It is not truthly recessive
            except KeyError:
                BS2_contrib.append(0)
        elif gene in dom_list:  # Then check dominant 
            key_snv = ref + '_' + alt # in case the name of the gene considered has dom = 1 in 'BS2_rec_dom_ad.txt'
            try:
                if key_snv == BS2_hom_het_dict["het"][str(chr)][str(start)]:
                    BS2_contrib.append(1) # if it is dominant and heterozygous it is correct.
                else:
                    BS2_contrib.append(0)
            except KeyError:
                BS2_contrib.append(0)
            ...
            BS2_contrib.append(0) # in case there's no located either in rec_list or dom_list.
    return BS2_contrib
...
def check_vep_BS3(full_stuff):
    ...
    for rev, sig in zip(review_series, sig_series): ##Changed numerical clasification to categorical
        if (rev.split('&')[0] == 'reviewed_by_expert_panel' or rev.split('&')[0] == 'practice_guideline') and \
                (sig.split('&')[0] == 'Benign' or sig.split('&')[0] == 'Likely_benign' or sig.split('&')[0] == 'Benign/Likely_benign'):
...
def check_vep_BP6(full_stuff):
    path_series = full_stuff['clinvar_clnsig']
    BP6_contrib = []
    for pred in path_series: ##Changed numerical clasification to categorical
        if pred.split("&")[0] == "Benign" or pred.split("&")[0] == "Likely_benign" or pred.split('&')[0] == "Benign/Likely_benign":
...
def check_vep_PP3_and_BP4_dbnsfp(full_stuff):
    ...
    non_empty = [x for x in all_contrib if x != []] # considera las dimensiones de la matrix all_contrib?
